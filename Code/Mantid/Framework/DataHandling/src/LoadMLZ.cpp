...
// }
 ...
 /*
  * Maskeds detectors defined in the /Scan/instrument/Detector/pixel_mask
 */
// void LoadMLZ::maskDetectors(NeXus::NXEntry& entry) {
//     // path to the pixel_mask
//     std::string pmpath = "instrument/Detector/pixel_mask";

//     NeXus::NXData pmdataGroup = entry.openNXData(pmpath);
//     NeXus::NXInt pmdata = pmdataGroup.openIntData();

//     // load the list of masked detectors from the file into memory
//     pmdata.load();
//     std::vector<int> masked_detectors(pmdata(), pmdata()+pmdata.size());
                          ...
                          */
                          void LoadMLZ::loadDataIntoTheWorkSpace(NeXus::NXEntry& entry, int vanaCalculatedDetectorElasticPeakPosition)
                          {
                            ...
                            */
                            ...
                            double theoreticalElasticTOF = (m_mlzloader.calculateTOF(m_l1,m_wavelength) + m_mlzloader.calculateTOF(m_l2,m_wavelength))
                              * 1e6; //microsecs
